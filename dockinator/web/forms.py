...
class KillContainerForm(forms.Form):
    container_id = forms.CharField(widget=forms.HiddenInput())
    magic_key = forms.CharField(widget=forms.HiddenInput())

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["container_id"] = forms.CharField(widget=forms.HiddenInput())
        self.fields["magic_key"] = forms.CharField(widget=forms.HiddenInput())
    ...
    def clean(self):
        ...
        if not DockerContainer.objects.filter(
            id=container_id, allocated_to=user
        ).exists():
