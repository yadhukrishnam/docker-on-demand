...
class DeployPageView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        form = DeployContainerForm(
            request.POST, running_image_ids=running_image_ids, required_result=required_result)
        ...
        if form.is_valid():
            ...
            r = requests.post("http://localhost:8000/api/images/deploy/",
...
class KillPageView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        magic_key = kwargs['magic_link']
        user = get_object_or_404(User, magic_key=magic_key, is_active=True)
        container = get_object_or_404(DockerContainer, container_id=kwargs['container_id'], allocated_to_id=user, killed_at=None)

        r = requests.delete(f"http://localhost:8000/api/container/{container.container_id}/")
