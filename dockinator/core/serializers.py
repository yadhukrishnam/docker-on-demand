...
from core.utils import deploy_container, kill_container, is_image_available
...
class DockerImageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DockerImage
        fields = ['id', 'name', 'tag', 'is_enabled', 'port_to_expose',
                  'allocated_port_start', 'allocated_port_end', 'lifespan', 'created_time']

    def create(self, validated_data):
        if not is_image_available(validated_data.get('name'), validated_data.get('tag')):
...
class DockerContainerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'container_id', 'container_name', 'image',
                  'allocated_to', 'assigned_port', 'created_time', 'killed_at']
    ...
    def create(self, validated_data):
        ...
        assigned_port = next((port for port in port_range if not DockerContainer.objects.filter(
            assigned_port=port, killed_at=None).exists()), None)
        container_name = f"{image.name}-{user.name}-{assigned_port}"
        container_id = deploy_container(
            f"{image.name}:{image.tag}", container_name, image.port_to_expose, assigned_port)
        ...
        print("Assigned port: ", assigned_port,
              " for internal container_id: ", container_id, "for user", user.name)
        return DockerContainer.objects.create(assigned_port=assigned_port, container_name=container_name, container_id=container_id, **validated_data)
